...
const favoriteBlog = blogs => {
  ...
  if (blogs.length === 0) {
    ...
    const mostLikedBlog = blogs.reduce((tykatyin, blogi) => {
      return tykatyin.likes <= blogi.likes ? blogi : tykatyin
    }, blogs[0])
  ...
  if (blogs.length === 0) {
    ...
    const result = authorLikes.reduce(
      ...
      },
      {
        ...
        likes: authorLikes
          .filter(x => x.author === authorLikes[0].author)
          .map(x => x.likes)
          .reduce((sum, x) => (sum += x), 0),
      }
    )
